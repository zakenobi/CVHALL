...
def get_processed_image(img, net, confThreshold, nmsThreshold):
    ...
    if temp == True :
        if i%50==0:
            max_temp=get_temp()
        img = cv2.rectangle(img,(480/2, 270/2),(1440/2,810/2),(255,0,0),2)
        text = f'Temp : {max_temp}'
        cv2.putText(img, text, (480/2, 260/2), cv2.FONT_ITALIC, 0.6,(255, 0, 0),1)
...
class Camera(QTimer):
    ...
    def send_email(self):
        ...
        mail.send(receiver='zac.gagnou@gmail.com', subject='ALERT', message='Individus dangereux demasqués !')
